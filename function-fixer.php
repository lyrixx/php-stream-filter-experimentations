...
final class StreamWrapper
{
    ...
    public function stream_stat(): array
    {
        ...
        // StreamFilter::doFilter(file_get_contents($this->context->path), $this->context);

        // $stat["size"] = $stat["size"] + $this->context->addedLength;
        unset($stat['size']);
...
class StreamFilter extends php_user_filter
{
    ...
    public static function doFilter(string $buffer, StreamContext $context): string
    {
        ...
        $suffix = '_'.bin2hex(random_bytes(10));
        $suffixLen = strlen($suffix);

        $count = 0;
        $buffer = str_replace("foobar", "foobar$suffix", $buffer, $count);
        $context->addedLength += $count * $suffixLen;

        return $context->content = $buffer;
    }
}

FunctionFixer::register();
